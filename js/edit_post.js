document.addEventListener('DOMContentLoaded', function() {
    ...
    if (langToggle) {
        langToggle.addEventListener('change', function() {
            // Lấy các tham số URL hiện tại (ví dụ: ?id=123)
            const urlParams = new URLSearchParams(window.location.search);

            if (this.checked) {
                // Thiết lập tham số 'lang' thành 'en'
                urlParams.set('lang', 'en');
            } else {
                // Thiết lập tham số 'lang' thành 'vi'
                urlParams.set('lang', 'vi');
            }

            // Tải lại trang với tham số đã được cập nhật (ví dụ: ?id=123&lang=en)
