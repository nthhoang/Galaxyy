...
if($isGroupPost){
    ...
    $reactionTableName="group_reaction";
    $commentTableName="group_comments";
}else{
    $postTableName = "posts";
...
try {
    ...
    $stmt_get_media = $conn->prepare("SELECT file_path FROM  $postMediaTableName WHERE post_id = ?");
    ...
    $stmt_delete_media = $conn->prepare("DELETE FROM  $postMediaTableName WHERE post_id = ?");
    ...
    $stmt_delete_reactions = $conn->prepare("DELETE FROM $reactionTableName WHERE post_id = ?");
    $stmt_delete_reactions->bind_param("i", $post_id);
    $stmt_delete_reactions->execute();

    $stmt_delete_comments = $conn->prepare("DELETE FROM $commentTableName WHERE post_id = ?");
    $stmt_delete_comments->bind_param("i", $post_id);
    $stmt_delete_comments->execute();

    // 8. Xóa bài đăng
    $stmt_delete_post = $conn->prepare("DELETE FROM $postTableName WHERE id = ?");
    $stmt_delete_post->bind_param("i", $post_id);
    $stmt_delete_post->execute();

    $conn->commit();

    header("Location: nhom.php?status=deleted");
    exit();
